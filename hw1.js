...
const Product = class Product {
    constructor(arg){
        const prod=arg.split(',');
        this.name=prod[0];
        this.price=parseFloat(prod[1]);
        this.availability=prod[2];
    }
    /**
     * *****************
     * ***** TODO ******
     ...
     * Reference: https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array 
     */
    static halfOff = (products) => {return products.map(({name,price,availability})=>(
    {name:name, price: price * .5, availability: availability}));}
     ...
     */
    static printProducts = (products) => {products.forEach((product)=>console.log(
    'Product: '+ product.name
    + ', Cost: '+ new Intl.NumberFormat("en-US",{style: "currency", currency:"USD",maximumFractionDigits: 2}).format(product.price) 
    + ', Availability: ' + (product.availability === 'In Stock'? 'Yes':'No')));}
